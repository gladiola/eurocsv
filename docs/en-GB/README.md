# EuroCSV — CSV Localization Converter

EuroCSV helps teams convert CSV files between international business formats. It adjusts delimiters, decimal separators, thousands separators, and date formats so data imports cleanly into the destination locale.

> **Note:** en-GB uses `,` as the field delimiter, `.` as the decimal separator, `,` as the thousands separator, and `dd/MM/yyyy` as the standard date format.
...
Upload your CSV file, select the source and target locale, then download the converted file.
