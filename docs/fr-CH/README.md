# EuroCSV — Convertisseur CSV de localisation

EuroCSV aide les équipes à convertir des fichiers CSV entre différents formats internationaux. L'application adapte les délimiteurs, les séparateurs décimaux, les séparateurs de milliers et les formats de date afin que les données s'importent correctement dans la région cible.

> **Remarque:** fr-CH utilise généralement `;` comme délimiteur de champs, `.` comme séparateur décimal, `'` comme séparateur de milliers et `dd.MM.yyyy` comme format de date.

---

## Conventions CSV par paramètres régionaux

| Paramètres régionaux | Délimiteur | Décimal | Milliers | Format de date |
|---|---|---|---|---|
...
Téléversez votre fichier CSV, sélectionnez les paramètres régionaux source et cible, puis téléchargez le fichier converti.
